...
@app.route("/fav/<int:place_id>", methods=["POST"])
def toggle_fav(place_id):
    conn = db.connect()
    db.toggle_favorite(conn, session["user_id"], place_id)
    conn.close()
    # 누른 자리로 돌아간다 (지도에서 눌렀으면 지도로, 상세에서 눌렀으면 상세로)
    return redirect(request.form.get("back") or url_for("panel", place_id=place_id))
...
def favorites():
    conn = db.connect()
    places = db.favorite_places(conn, session["user_id"])
    conn.close()
    # 확장 3 — 내 공유 코드. 짝꿍이 이 번호를 넣으면 찜을 합쳐 동선을 짠다.
    return render_template("favorites.html", places=places,
                           share_code=session["user_id"])
...
def make_route():
    ...
    # 확장 3 — 짝꿍의 공유 코드를 받으면 두 사람의 찜을 합집합으로 모은다.
    partner = (request.form.get("merge") or "").strip()
    partner_id, partner_error = None, None
    if partner:
        if partner.isdigit() and db.user_exists(conn, int(partner))                 and int(partner) != session["user_id"]:
            partner_id = int(partner)
        else:
            partner_error = partner

    if partner_id:
        places = db.merged_favorite_places(conn, [session["user_id"], partner_id])
    else:
        places = db.favorite_places(conn, session["user_id"])
    conn.close()

    if len(places) < 2:
        # 찜이 0~1곳이면 정렬할 것이 없다. 빈 화면 대신 안내를 보여 준다.
        return render_template("route.html", plan=None, places=places,
                               partner_id=partner_id, partner_error=partner_error)

    plan = route_lib.route_plan(places)
    svg_points = geo.project_points(
        plan["order"], focus=plan["order"], width=520, height=440, min_dist=34)
    # 그림 위 번호는 방문 순서와 같아야 한다
    order_no = {p["id"]: i + 1 for i, p in enumerate(plan["order"])}
    for pt in svg_points:
        pt["order"] = order_no[pt["id"]]
    svg_points.sort(key=lambda p: p["order"])

    return render_template("route.html", plan=plan, places=places,
                           svg_points=svg_points,
                           partner_id=partner_id, partner_error=partner_error)


# ═══════════════════════════════════════════════════════════════
# 6. 통계 — 보정 전/후 랭킹 비교 (수업 데모의 클라이맥스)
# ═══════════════════════════════════════════════════════════════
...
def stats():
    ...
    compare = scoring.rank_compare(places)
    flips = [r for r in compare if abs(r["delta"]) >= 3]

    # 카테고리별 리뷰 수 분포 (CSS 막대로 그린다 — 차트 라이브러리 없음)
    dist = {}
    for p in places:
        d = dist.setdefault(p["category"], {"n": 0, "visitor": 0})
        d["n"] += 1
        d["visitor"] += p["visitor_rev"]
    max_visitor = max((d["visitor"] for d in dist.values()), default=1)

    interests, log_count = user_taste(conn)
    summary = db.click_summary(conn, session["user_id"])
    conn.close()

    for r in compare:
        r["s_star"] = scoring.star_score(r["adj_star"], meta["adj_min"], meta["adj_max"])

    # 가장 크게 움직인 한 곳 — 발표 때 학생이 설명해야 하는 장소다
    biggest = max(compare, key=lambda r: abs(r["delta"])) if compare else None

    return render_template(
        "stats.html", compare=compare, flips=flips, biggest=biggest,
        dist=dist, max_visitor=max_visitor,
        interests=interests, log_count=log_count, summary=summary,
...
if __name__ == "__main__":
    # 포트는 기본 5000이다. 이미 쓰고 있는 프로그램이 있으면(자매 프로젝트 v2.4 등)
    # PORT 환경변수로 바꿔 띄운다.  예)  PORT=5050 python app.py
    port = int(os.environ.get("PORT", 5000))

    print(f"지도 방식 : {MAP_MODE}   (leaflet 번들 {'있음' if HAS_LEAFLET else '없음'}"
          f" / 타일서버 {'연결됨' if ONLINE else '연결 안 됨'})")
    print(f"http://127.0.0.1:{port} 을 브라우저에서 여세요")
