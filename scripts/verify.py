# -*- coding: utf-8 -*-
"""
verify.py — 수업 전날 이것 하나로 전부 확인한다

기획안 §10의 13항목(0~12) + 확장 과제 회귀 3항목(13~15).
확장 회귀는 "확장을 켜지 않으면 기본 과제와 완전히 같다"를 보증한다(§13).
...
def main():
    ...
    # ── 확장 과제 회귀 (§13) — "기본값 꺼짐"이 지켜지는지 ──────────
    def c13():
        """아무 인자도 주지 않으면 기본 과제와 완전히 같아야 한다."""
        import app as app_module
        client = app_module.app.test_client()
        import re
        def order(url):
            html = client.get(url).get_data(as_text=True)
            return re.findall(r'href="/panel/(\d+)"', html)
        base = order("/")
        off = order("/?use_blog=0&heat=0")
        return ("PASS" if base == off and base else "FAIL",
                f"기본 {len(base)}곳 · 확장 끈 상태와 순서 {'동일' if base == off else '다름'}")

    def c14():
        """확장 2 — 블로그리뷰를 더하면 v가 커져 값이 달라진다."""
        sample = next(p for p in places if p["blog_rev"] > 0)
        plain = scoring.adjusted_star(sample["star"], sample["visitor_rev"], m, C)
        with_blog = scoring.adjusted_star(
            sample["star"], sample["visitor_rev"], m, C,
            blog_rev=sample["blog_rev"], use_blog=True)
        stored = abs(sample["adj_star"] - plain) < 1e-9 and             abs(sample["adj_star_blog"] - with_blog) < 1e-9
        return ("PASS" if stored else "FAIL",
                f"{sample['name']}: 기본 {plain:.3f} / 블로그 반영 {with_blog:.3f} "
                f"(DB 저장값과 {'일치' if stored else '불일치'})")

    def c15():
        """확장 3 — 없는 짝꿍 코드를 넣어도 내 찜만으로 정상 동작한다."""
        import app as app_module
        client = app_module.app.test_client()
        client.post("/fav/1", data={"back": "/favorites"})
        client.post("/fav/2", data={"back": "/favorites"})
        bad = client.post("/route", data={"merge": "999999"})
        good = client.post("/route", data={"merge": ""})
        ok = bad.status_code == 200 and good.status_code == 200             and "없는 코드" in bad.get_data(as_text=True)
        return ("PASS" if ok else "FAIL",
                f"잘못된 코드 {bad.status_code} · 빈 값 {good.status_code}")
    ...
    checks = [
        ...
        (13, "[확장] 인자를 안 주면 기본 과제와 동일", c13),
        (14, "[확장 2] 블로그리뷰 반영값이 따로 저장돼 있다", c14),
        (15, "[확장 3] 잘못된 짝꿍 코드도 안전하게 처리", c15),
    ]
